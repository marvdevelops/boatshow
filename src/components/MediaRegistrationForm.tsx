...
export function MediaRegistrationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const [businessCardUpload, mediaIdUpload] = await Promise.all([
        uploadFile(businessCard, 'Media', tempId),
        uploadFile(mediaId, 'Media', tempId),
      ]);

      // Create submission with file URLs
      const submissionData = {
        ...
        mediaId: mediaIdUpload.url,
        mediaIdPath: mediaIdUpload.path,
        category: 'Media',
      };
  ...
  if (submitted) {
    return (
      <FormLayout title="Media Registration">
        <div className="text-center py-12">
          <CheckCircle className="w-16 h-16 text-green-500 mx-auto mb-4" />
          <h2 className="text-[#0A2647] mb-4">Thank You!</h2>
          <p className="text-gray-600">
            Your media application has been submitted for verification.
          </p>
    ...
    <FormLayout title="Media Registration">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <FileUpload
            label="Media ID for Verification"
            required
            onChange={setMediaId}
            accept=".pdf,.png,.jpg,.jpeg"
          />
